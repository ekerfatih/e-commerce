...
const CategorySquare = ({image, category, amount}) => {
    return (
        <div>
            <div className={"relative h-full w-full font-montserrat"}>
                <img className={"aspect-square w-full h-full object-cover"} src={image} alt=""/>
                <div className={"bg-black/40 hover:bg-transparent z-1 w-full h-full absolute top-0 left-0"}></div>
                <h5 className={"text-white font-bold transform -translate-x-1/2 -translate-y-1/2 text-center left-1/2 top-1/2 absolute z-10"}>{category}</h5>
